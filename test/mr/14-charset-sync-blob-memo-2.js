...
fb.attach(options, async (err, db) => {
    if (err) throw err;

        let txOrDb = db;
        if (useTx) {
            txOrDb = await new Promise((resolve, reject) => {
                db.startTransaction(ISOLATION_READ_COMMITED, (err, tx) => {
                    if (err) {
                        return reject(err);
                    }
                    resolve(tx);
        ...
        txOrDb.query(`SELECT first ${fetchSize} * FROM ${'PSALE_ARTICLE'} WHERE PSART_ID=?`, [1003001237675], async (err, rows) => {
            ...
            /*
             ...
             ⚠️ db.query() != from tx.query()
                ^ fetch_blob_sync
                                    ^ fetch_blob_async
            */
            if (useTx && !process.argv.includes('--skip-blob-fetching')) {
