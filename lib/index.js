...
(function(){
	...
	async function readBlob(col, options) {
		...
		options = options || {};
		options.async =  typeof options.async === 'boolean' ? options.async : true;
		...
		const fetchBlob = (col) => {
			return new Promise((resolve, reject) => {
				if (options.async) {
					if (!this._tx) {
						return reject(new Error('Transaction is undefined.'))
					}
					col(this._tx, function (err, name, e) {
						if (err) {
							reject(err);
						}
	...
	function Db(originalDb, opts) {
		...
		this.readBlob = (...args) => {
			const opts = args[1] ?? {};
			opts.async = false; // async is not allowed here
			return readblob(args[0], opts);
		};
	}
