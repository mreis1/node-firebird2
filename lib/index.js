...
Fb2.prototype.Pool = (max, options) => {
...
Fb2.prototype.Transaction = class Transaction {
	_tx;
	constructor(tx) {
		this._tx = tx;
	}
	...
	query(query, params) {
		return new Promise((resolve, reject) => {
			this._tx.query(query, params, (err, result) => {
				return new Promise((resolve, reject) => {
...
Fb2.prototype.Db = class Db {
	...
	query(sql, params) {
		return new Promise((resolve, reject) => {
			this._db.query(sql, params, function (err, result) {
				err ? reject(err) : resolve(result);
			})
		})
	}
	...
	transaction(isolation) {
		return new Promise((resolve, reject) => {
			this._db.transaction(function (err, tx) {
				if (err) {
					reject(err)
				}
				else {
					resolve(new Fb2.Transaction(tx))
				}
			})
		})
	}
...
Fb2.prototype.attach = function (options) {
	return new Promise((resolve, reject) => {
