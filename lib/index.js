...
const DEFAULT_TRANSCODE_ADAPTER = {
	text: {
		fromDb: function (buffer) {
			return buffer;
		},
		toDb: function (value) {
...
function normalizeTranscodeAdapter(adapter) {
	const text = adapter && adapter.text ? adapter.text : {};
	return {
		text: {
			fromDb: typeof text.fromDb === 'function' ? text.fromDb : DEFAULT_TRANSCODE_ADAPTER.text.fromDb,
			toDb: typeof text.toDb === 'function' ? text.toDb : DEFAULT_TRANSCODE_ADAPTER.text.toDb
		}
	};
}
...
SQLVarText.prototype.decode = function (data) {
	...
	var useAdapter = connection && connection._isEncodingNone && this.subType !== 1;
	if (useAdapter) {
		ret = data.readBuffer(this.length) || Buffer.alloc(0);
		ret = connection._decodeTextFromDb(ret, this);
...
SQLVarString.prototype.decode = function (data) {
	...
	var useAdapter = connection && connection._isEncodingNone && this.subType !== 1;
	if (useAdapter) {
		ret = data.readBuffer() || Buffer.alloc(0);
		ret = connection._decodeTextFromDb(ret, this);
	} else if (this.subType > 1) {
...
SQLParamString.prototype.calcBlr = function (blr) {
	...
	if (Buffer.isBuffer(this.value)) {
		...
		len = this.value ? Buffer.byteLength(this.value, DEFAULT_ENCODING) : 0;
	}
	blr.addWord(len);
};
...
function search_blob_column(row, meta) {
	...
	for (var column in meta) {
		if (meta[column] instanceof SQLVarBlob) {
			result.push({
				key: asObject[column],
				index: Number(column),
				subType: meta[column].subType
			});
		}
	}
	return result;
...
var fetch_blob_sync = exports.fetch_blob_sync = function(rows, meta, isSelect, callback, connection) {
	var _rows = rows;
	...
	function decodeBlob(data, colMeta) {
		if (data === null || data === undefined)
			return data;
		if (connection && connection._isEncodingNone && colMeta.subType === 1) {
			return connection._decodeTextFromDb(data || Buffer.alloc(0), meta[colMeta.index]);
		}
		return data;
	}
	...
	var parallel = function(group, size) {
		...
		prows.forEach(function (row) {
			blob_col.forEach(function (col) {
				readblob(row[col.key], function (err, data) {
					if (err)
						blob_err = err;
					nb_blobs--;
					p_nb_blobs--;
					try {
						row[col.key] = decodeBlob(data, col);
					} catch (e) {
						blob_err = blob_err || e;
						row[col.key] = data;
					}
...
function fetch_blob_async(statement, transaction, id, name, fieldMeta) {
	if (!id)
		return null;
	...
	return function (callback) {
		...
		statement.connection.openBlob(id, transaction, function (err, blob) {
			...
			var connection = statement.connection;
			var isTextBlob = connection && typeof connection._decodeTextFromDb === 'function' && fieldMeta && fieldMeta.subType === 1;
			var textChunks = isTextBlob ? [] : null;
			var textIsBuffer = true;
			...
			function read() {
				statement.connection.getSegment(blob, function (err, ret) {
					...
					if (ret.buffer) {
						...
						if (data && isTextBlob) {
							emitted = connection._decodeTextFromDb(data, fieldMeta);
							textChunks.push(emitted);
							if (!Buffer.isBuffer(emitted))
								textIsBuffer = false;
						}
					...
					if (isTextBlob) {
						try {
							...
							else if (textIsBuffer)
								decoded = Buffer.concat(textChunks);
							else
								decoded = textChunks.join('');
							e.decoded = decoded;
							e.emit('text', decoded);
						}
						catch (err) {
...
var Connection = exports.Connection = function (host, port, callback, options, db, svc) {
	...
	this.options = options;
	var encoding = (options.encoding || 'UTF8').toUpperCase();
	this._encoding = encoding;
	this._isEncodingNone = encoding === 'NONE';
	this._transcodeAdapter = normalizeTranscodeAdapter(options.transcodeAdapter);
	this._bind_events(host, port, callback);
	this.error;
...
Connection.prototype._decodeTextFromDb = function (buffer, field) {
	var adapter = this._transcodeAdapter && this._transcodeAdapter.text && this._transcodeAdapter.text.fromDb;
	if (typeof adapter === 'function') {
		return adapter(buffer, field);
	}
	return buffer;
};

Connection.prototype._encodeTextToDb = function (value, field) {
	var adapter = this._transcodeAdapter && this._transcodeAdapter.text && this._transcodeAdapter.text.toDb;
	var buff = adapter ? adapter(value, field) : DEFAULT_TRANSCODE_ADAPTER.text.toDb(value);
    console.log('adapter buff', value.substring(0, 50))
	if (!Buffer.isBuffer(buff)) {
		throw new Error('transcodeAdapter.text.toDb must return a Buffer');
	}
	return buff;
};
						...
						try {
							...
							if (custom.asObject) {
									if (item.type === SQL_BLOB)
										value = fetch_blob_async(statement, transaction, value, data.fcols[data.fcolumn], item);
								data.frow[data.fcols[data.fcolumn]] = value;
							}
							else {
								if (item.type === SQL_BLOB)
									value = fetch_blob_async(statement, transaction, value, data.fcolumn, item);
...
Connection.prototype.attach = function (options, callback, db) {
	...
	var encoding = (options.encoding || this._encoding || 'UTF8').toUpperCase();
	this._encoding = encoding;
	this._isEncodingNone = encoding === 'NONE';
	this._transcodeAdapter = normalizeTranscodeAdapter(options.transcodeAdapter || this._transcodeAdapter);
	...
	blr.addByte(1);
	blr.addString(isc_dpb_lc_ctype, encoding, DEFAULT_ENCODING);
...
Connection.prototype.createDatabase = function (options, callback) {
	...
	blr.pos = 0;
	blr.addByte(1);
	blr.addString(isc_dpb_set_db_charset, encoding, DEFAULT_ENCODING);
	blr.addString(isc_dpb_lc_ctype, encoding, DEFAULT_ENCODING);
	blr.addString(isc_dpb_user_name, user, DEFAULT_ENCODING);
	blr.addString(isc_dpb_password, password, DEFAULT_ENCODING);
...
Connection.prototype.startTransaction = function (isolation, callback) {
	...
	const targetIsolation = new Isolation(isolation); // use isolation provieded during the connection if (defined)
				...
				while (!finishDescribe) {
					switch (br.readByteCode()) {
						...
						case isc_info_sql_type:
							...
							param.connection = statement.connection;
							param.type = type;
...
Connection.prototype.prepareStatement = function (transaction, statement, query, plan, callback) {
	...
	var queryEncoding = DEFAULT_ENCODING;

	if (this._isEncodingNone) {
		var encodedQuery = this._encodeTextToDb(query);
		queryPayload = encodedQuery.toString('binary');
		queryEncoding = 'binary';
	}

	msg.addString(queryPayload, queryEncoding);
	msg.addBlr(blr);
...
Connection.prototype.executeStatement = function (transaction, statement, params, callback, custom) {
	...
	function PrepareParams(params, input, callback) {
		...
		function putBlobData(index, value, meta, callback) {

			self.createBlob2(transaction, function (err, blob) {

				var b;
				var isStream = value && value.readable;
				var hasTextAdapter = self._transcodeAdapter && self._transcodeAdapter.text && typeof self._transcodeAdapter.text.fromDb === 'function' && typeof self._transcodeAdapter.text.toDb === 'function';
				var isTextBlob = meta && meta.subType === 1 && hasTextAdapter;

				if (Buffer.isBuffer(value) && !isTextBlob)
					b = value;
				else
				if (typeof (value) === 'string')
					b = isTextBlob ? self._encodeTextToDb(value, meta) : Buffer.from(value, DEFAULT_ENCODING)
				else
				if (!isStream) {
					var strValue = JSON.stringify(value);
					b = isTextBlob ? self._encodeTextToDb(strValue, meta) : Buffer.from(strValue, DEFAULT_ENCODING)
				}
				...
				value.on('data', function (chunk) {
					...
					if (isTextBlob) {
						var strChunk = typeof payload === 'string' ? payload : payload.toString(DEFAULT_ENCODING);
						payload = self._encodeTextToDb(strChunk, meta);
					}

					bufferReader(payload, 1024, function (b, next) {
						self.batchSegments(blob, b, next);
					}, function () {
						isReading = false;
					...
					default:
						switch (typeof value) {
							case 'number':
								...
								break;
							case 'string':
								if (self._isEncodingNone && (meta.type === SQL_TEXT || meta.type === SQL_VARYING) && meta.subType !== 1) {
									ret[i] = new SQLParamString(self._encodeTextToDb(value, meta));
								} else {
							...
							case 'boolean':
								...
								break;
							default:
								if (Buffer.isBuffer(value) && (meta.type === SQL_TEXT || meta.type === SQL_VARYING)) {
									ret[i] = new SQLParamString(value);
								}
								else {
