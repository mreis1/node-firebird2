...
exports.DEFAULT_ISOLATION = ISOLATION_REPEATABLE_READ;
...
Connection.prototype.startTransaction = function (isolation, callback) {
	...
	blr.addBytes(isolation || this._isolation);
...
(function(){
	...
	async function readBlob(col, options) {
		...
		if (!(col === null || colType === undefined || colType === 'string' || Buffer.isBuffer(col) || colType === 'function')) {
	...
	function Db(originalDb, opts) {
		...
		/**
		 * @param isolation     The firebird isolation
		 ...
		 * @returns {Promise<Fb2.Transaction>}
		 */
		this.transaction = function(isolation, opts) {
			return new Promise((resolve, reject) => {
				originalDb.transaction(isolation, function (err, originalTx) {
					if (err) {
						reject(err)
					}
					else {
						resolve(new Fb2.Transaction(originalTx, opts))
					}
				})
			})
		}
