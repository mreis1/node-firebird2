...
const
	...
	ISOLATION_READ_COMMITED_READ_ONLY = [ISC_TPB.version3, ISC_TPB.read, ISC_TPB.wait, ISC_TPB.read_committed, ISC_TPB.no_rec_version];
	/*NEW*/ISOLATION_READ_COMMITED_NOWAIT = [ISC_TPB.version3, ISC_TPB.write, ISC_TPB.nowait, ISC_TPB.read_committed, ISC_TPB.no_rec_version];
	/*NEW*/ISOLATION_READ_COMMITED_READ_ONLY_NOWAIT = [ISC_TPB.version3, ISC_TPB.read, ISC_TPB.nowait, ISC_TPB.read_committed, ISC_TPB.rec_version];
...
exports.ISOLATION_READ_COMMITED_READ_ONLY_NOWAIT = ISOLATION_READ_COMMITED_READ_ONLY_NOWAIT;
exports.DEFAULT_ISOLATION = ISOLATION_READ_COMMITED_READ_ONLY_NOWAIT;
...
/**
 * @param isolationOrOptions {number[] | object}
 * 			isolation: ISOLATION_<TYPE> (See Firebird.ISOLATION_READ_COMMITED and other...)
 * 			lockTimeout (optional) number | Infinity - Prescribes the maximum number of seconds that the transaction should wait when a lock conflict occurs
 *  					(default: 15) - Use infinity to hold until locking resource gets released
 * 						https://firebirdsql.org/refdocs/langrefupd21-set-trans.html#langrefupd21-set-trans-lock-timeout
 *			rec: (optional) boolean | undefined (default = undefined) When undefined the transaction preset is used.
 *			wait: (optional) boolean | default = undefined: When undefined the transaction preset is used
 * @param callback
 *
...
Connection.prototype.startTransaction = function (isolationOrOptions, callback) {
	...
	if (typeof (isolation) === 'function') {
		...
		isolation = callback;
		callback = tmp;
	} else if (Array.isArray(isolationOrOptions)) {
		isolation = isolationOrOptions;
	} else {
		const {isolation: _isolation, ..._options} = isolationOrOptions;
	...
	const targetIsolation = (isolation || this._isolation).map(val => val); // copy
	if (options.mode === 'write' || options.mode === 'read') {
		targetIsolation[1] = options.mode === 'write' ? ISC_TPB.write : ISC_TPB.read;
	}
	if (typeof options.rec === 'boolean') {
		targetIsolation[4] = options.rec ? ISC_TPB.rec_version : ISC_TPB.no_rec_version;
	}
	if (typeof options.wait === 'boolean') {
		targetIsolation[2] = options.wait ? ISC_TPB.wait : ISC_TPB.nowait;
	...
	blr.addBytes(targetIsolation);
	// Only apply the lockTimeout if is a `writeable` transaction with `wait` flag.
	if (hasLockTimeout && targetIsolation[1] === ISC_TPB.write && targetIsolation[2] === ISC_TPB.wait) {
		blr.addByte(ISC_TPB.lock_timeout)
		blr.addByte(4)
		blr.addInt32(options.lockTimeout);
	}
...
(function(){
	...
	const upgradeTx = (context) => _startTx(ISOLATION_READ_COMMITED, context);
	const restartTx = (context, isolation) => _startTx(isolation || 'auto', context);
	const _startTx = (isolation, context) => {
		isolation = isolation || 'auto';
		return new Promise((resolve, reject) => {
			try {
                log('Transaction is being upgraded...');
				context._tx.commit();
			} catch (err) {
				log('Failed to commit an active transaction during restart: ' + err?.message);
			}
			const targetIsolation = isolation === 'auto' ? (context._tx.connection._isolation ?? exports.DEFAULT_ISOLATION) : isolation;
			context._tx.db.transaction(targetIsolation , (err, tx) => {
				...
				log('Transaction successfully upgraded');
