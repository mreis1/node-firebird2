			...
			function read() {
				statement.connection.getSegment(blob, function (err, ret) {
					...
					if (ret.buffer) {
						...
						if (data && isTextBlob) {
                            const text = connection._decodeTextFromDb(data, fieldMeta);
							emitted = Buffer.from(text, 'utf8');
        ...
        const fetchBlob = (col) => {
            return new Promise((resolve, reject) => {
                col(function (err, name, e) {
                    ...
                    let isText = false;
                    // e === EventEmitter
                    e.on('text', function (value/*Buffer*/) {
                        isText = true;
                        // i'm receiving text. This makes preprocessBuffer useless / deprecated
