...
exports.DEFAULT_ISOLATION = ISOLATION_READ_COMMITED_READ_ONLY;
...
var Connection = exports.Connection = function (host, port, callback, options, db, svc) {
	...
	this._isolation = options.isolation||exports.DEFAULT_ISOLATION;
};
 ...
 */
Connection.prototype.startTransaction = function (isolationOrOptions, callback) {
	if (typeof callback !== 'function') {
		throw new TypeError('callback must be a function');
	}
	let {isolation, wait, lockTimeout} = new Isolation(isolationOrOptions);
...
class Isolation {
	...
	constructor(isolationOrOptions) {
		...
		/**
		 * Note: 'write' transactions will use rec_version if rec is not specified
		 * 		 'read' transactions  will use no_rec_version
		 *
		 */
		if (options.mode === 'write' || options.mode === 'read') {
			isolation[1] = options.mode === 'write' ? ISC_TPB.write : ISC_TPB.read;
			options.rec ??= options.mode === 'write';
			options.wait ??= options.mode === 'write';
		}
...
(function(){
	...
	const upgradeTx = (context) => _startTx(new Isolation({mode: 'write'}, context, true), context);
	const restartTx = (context, isolation) => _startTx(isolation || 'auto', context);
	const _startTx = (isolation, context, isUpgrade) => {
		isolation = isolation || 'auto';
		return new Promise((resolve, reject) => {
			if (context?._tx) {
				try {
					log('Commiting old transaction...');
					context._tx.commit();
				} catch (err) {
					log('Failed to commit old transaction during tx restart: ' + err?.message);
				}
			}
			try {
				const targetIsolation = isolation === 'auto' ? context._tx.connection._isolation : isolation;
				context._tx.db.transaction(targetIsolation , (err, tx) => {
					if (err) {
						log('Transaction could not be upgraded due to unexpected error: '+ err?.message);
						return reject(err);
					}
