...
/**
 * Schedules the callback operation for the active connection
 * The operation will be threaded sequentially upon socket.on('data')
 *
 * @param callback (optional) Ex: statement.release(<callback>) is often called without specifying a callback
 * @private
 */
...
function describe(ret, statement) {
	...
	// (MR)
	// br.buffer and input should always be defined.
	// but if some reason they are undefined, that would lead to a potential uncaught error.
	// for precaution we use optional chaining to access the length of `br.buffer` and the length of `input`
	// https://github.com/mreis1/node-firebird2/issues/1
...
(function(){
	...
	const upgradeTxRequired = (err, upgradable) => upgradable && err?.message?.includes('attempted update during read-only transaction');
	/**
	...
	const _startTx = (isolation, context, isUpgrade) => {
		return new Promise((resolve, reject) => {
			if (context?._tx) {
				try {
					log('[_startTx] Attempting to commit existing transaction.');
					context._tx.commit();
					log('[_startTx] Successfully commited old transaction.');
				} catch (err) {
					log('[_startTx] Old transaction could not be upgraded: ' + (err?.message ?? '??'));
				}
			}
			try {
				/**
				...
				// when context refers to `Fb.promises.Db` it's highly possible that _tx is undefined
				// @todo This needs additional testing - see 12_bug_insert_returning_into_fk_hang.js for a cenario
				// where current implementation is not working as expected.
				// (context._tx ? context._tx.db : context).startTransaction(targetIsolation , (err, tx) => {
				setTimeout(() => {
	...
	function Transaction(originalTx, opts) {
		...
		this.query = function(query, params) {
			return new Promise((resolve, __reject) => {
				const _reject = (err) => {
					// @todo: setTimeout to workaround invalid handle at 12_bug_insert_returning_into_fk if `await delay(0)` on line 32 is not present
					// setTimeout(() => {
						__reject(err);
					// },100) // @todo <--- Hack for invalid handle
				}
				const reject = (err) => {
