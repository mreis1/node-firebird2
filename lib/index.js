 ...
 * 			lockTimeout (optional) number (default: will hold forever until lock element is released)
 * 						https://firebirdsql.org/refdocs/langrefupd21-set-trans.html#langrefupd21-set-trans-lock-timeout
 *			recVersion: (optional) boolean | undefined (default = undefined) When undefined the transaction preset is used.
 * @param callback
...
Connection.prototype.startTransaction = function (isolationOrOptions, callback) {
	...
	const hasLockTimeout = typeof options?.lockTimeout === "number" && options.lockTimeout >= 0;
	blr.addBytes(targetIsolation);
	if (hasLockTimeout) {
		console.log('XXX',targetIsolation.includes(ISC_TPB.write) && targetIsolation.includes(ISC_TPB.wait));
		if (options.lockTimeout >= 0 && targetIsolation[1] === ISC_TPB.write && targetIsolation[2] === ISC_TPB.wait) {
			console.log('Registering the timeout');
...
(function(){
	...
	const upgradeTx = (context) => _startTx(ISOLATION_READ_COMMITED_NOWAIT, context);
	...
	function Db(originalDb, opts) {
		...
		this.transaction = function(isolationOrTxOpts) {
			return new Promise((resolve, reject) => {
				log('Tx settings: ', isolationOrTxOpts);
