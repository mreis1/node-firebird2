	...
	const _startTx = (isolation, context, isUpgrade) => {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				(context._tx ? context._tx.db : context).transaction(targetIsolation , (err, tx) => {
					if (err) {
					...
					// context._tx.connection._isolation = targetIsolation; // update isolation reference in connection
					resolve(context);
				})
			} catch (err) {
				reject(err);
	...
	function Transaction(originalTx, opts) {
		...
		this.query = function(query, params) {
			return new Promise((resolve, __reject) => {
				const _reject = (err) => {
					// Solves invalid handle at 12_bug_insert_returning_into_fk if `await delay(0)` on line 32 is not present
					setTimeout(() => {
						__reject(err);
					})
				}
