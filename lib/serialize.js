...
XdrWriter.prototype.addBuffer = function (buf) {
    var len = buf.length;
    var alen = align(len);
    this.ensure(alen + 4);
    this.buffer.writeInt32BE(len, this.pos);
    ...
    this.pos += len;
    if (alen > len) {
        this.buffer.fill(0, this.pos, this.pos + (alen - len));
        this.pos += (alen - len);
    }
};
...
var XdrReader = exports.XdrReader = function (buffer, opts) {
    opts = opts || {};
    this.allowUnaligned = !!opts.allowUnaligned;
...
XdrReader.prototype.readBuffer = function (len) {
    ...
    var r = this.buffer.slice(this.pos, this.pos + len);
    this.pos += this.allowUnaligned ? len : align(len);
    return r;
};
